...
const Widgets = ({ articles }) => {
  return (
    <div className='hidden md:inline space-y-2 '>
      <div className='bg-white dark:bg-[#1D2226] py-2.5 rounded-lg space-y-2 w-11/12 overflow-hidden border border-gray-300 dark:border-none'>
        ...
        <div className='space-y-1'>
          {articles.slice(0, 5).map((article, i) => (
            <div
              key={i}
              className='flex items-center cursor-pointer hover:bg-black/10 dark:hover:bg-black/75 px-2.5 py-2.5'
            >
              <FiberManualRecordRounded className='!h-2 !w-2' />
              <div>
                <h4 className='max-w-xs font-medium text-xs truncate pr-10'>
                  {article.title}
                </h4>
                <TimeAgo
      ...
      {/* ads */}
      <div className='bg-white dark:bg-[#1D2226] w-11/12 h-64 px-2.5 sticky top-20 rounded-lg border border-gray-300 dark:border-none'>
        <div className='h-full w-full'>
          <Image src='https://rb.gy/kbfeaa' layout='fill' objectFit='contain' />
        </div>
      </div>
    </div>
  );
